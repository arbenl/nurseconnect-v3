...
// ── NextAuth rejection guard ────────────────────────────────────────
// V3 uses Better-Auth, not NextAuth. Reject legacy NextAuth vars.
const REJECTED_NEXTAUTH_VARS = ["NEXTAUTH_SECRET", "NEXTAUTH_URL"] as const;

for (const key of REJECTED_NEXTAUTH_VARS) {
  if (process.env[key]) {
    throw new Error(
      `[env] NextAuth is not supported in V3. Remove "${key}" from your environment. Use BETTER_AUTH_SECRET instead.`
    );
  }
}
