...
export async function POST(request: Request, props: { params: Promise<{ id: string }> }) {
  ...
  const context = createApiLogContext(request, `/api/admin/nurses/${params.id}/suspend`, {
    action: "admin.nurses.suspend",
  });
  ...
  try {
    ...
    const parsed = AdminSuspendNurseSchema.safeParse(json);
    if (!parsed.success) {
      const response = NextResponse.json({ error: "Invalid data", details: parsed.error }, { status: 400 });
      logApiFailure(actorContext, parsed.error, 400, startedAt, { source: "admin.nurses.suspend" });
      return withRequestId(response, context.requestId);
    }

    const { reason } = parsed.data;
    const nurse = await suspendNurseCredential({
    ...
    if (!nurse) {
      ...
      logApiFailure(actorContext, "Nurse not found", 404, startedAt, { source: "admin.nurses.suspend" });
      return withRequestId(response, context.requestId);
    }
    const response = NextResponse.json({ ok: true, status: nurse.status, nurse });
    logApiSuccess(actorContext, 200, startedAt, { source: "admin.nurses.suspend" });
    return withRequestId(response, context.requestId);
  } catch (error) {
    ...
    if (err?.name === "NurseCredentialValidationError") {
      ...
      logApiFailure(actorContext, error, 400, startedAt, { source: "admin.nurses.suspend" });
      return withRequestId(response, context.requestId);
    }
    if (err?.name === "NurseCredentialConflictError") {
      ...
      logApiFailure(actorContext, error, 409, startedAt, { source: "admin.nurses.suspend" });
      return withRequestId(response, context.requestId);
    }
    if (err?.name === "UnauthorizedError") {
      const response = NextResponse.json({ error: "Unauthorized" }, { status: 401 });
      logApiFailure(context, "Unauthorized", 401, startedAt, { source: "admin.nurses.suspend" });
      return withRequestId(response, context.requestId);
    }
    if (err?.name === "ForbiddenError") {
      const response = NextResponse.json({ error: "Forbidden" }, { status: 403 });
      logApiFailure(actorContext, "Forbidden", 403, startedAt, { source: "admin.nurses.suspend" });
      return withRequestId(response, context.requestId);
    }
    logApiFailure(actorContext, error, 500, startedAt, { source: "admin.nurses.suspend" });
