...
export async function POST(_: Request, { params }: Params) {
  ...
  const context = createApiLogContext(_, "/api/requests/[id]/enroute", {
    action: "request.enroute",
  });
  ...
  try {
    ...
    logApiSuccess(actorContext, 200, startedAt, { action: "enroute", requestId: params.id });
    return withRequestId(response, context.requestId);
  } catch (error) {
