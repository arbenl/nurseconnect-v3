...
| `NC-E1-03` | `review` | `default-tenant-backfill-plan` | Define expand/contract tenant migration. | Default tenant/backfill plan and observe-before-enforce mechanism are documented in `docs/runbooks/default-tenant-backfill-plan.md`, including default org/facility/jurisdiction bootstrap, table classification, out-of-band DB access evidence, and pause/rollback gates. | High |
