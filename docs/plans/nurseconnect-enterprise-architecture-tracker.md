...
| `NC-E2-03` | `ready` | `platform-authz` | Add in-process tenant/resource-aware policy functions. **Superseded scope note:** amended by `docs/plans/ENTERPRISE_UPGRADE_TRACKER.md` (Phase C) to include phantom-type guards (`AuthorizedTransition`, branded `PolicyDecision`); the amended row governs. | Policy matrix covers allow/deny/cross-tenant/PHI field cases. | High |
