...
| ID | Status | Slice | Work | Acceptance Criteria (falsifiable) | Risk |
|---|---|---|---|---|---|
| `NC-TB-01` | `ready` | `tenant-expand` | Default org/facility bootstrap; add **nullable** `organization_id` (+ `branch_id`) to all domain tables; backfill to default tenant. | Migration is reversible; zero NULL `organization_id` after backfill (invariant test); no query behavior change (full gate:release green). | High |
