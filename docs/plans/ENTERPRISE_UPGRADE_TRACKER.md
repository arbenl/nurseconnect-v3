...
| `NC-E2-04` | `ready` | `medical-evidence-brand` | `MedicalEvidence` brand for credential/clinical state: `nurses.status` transitions (draft→verified→suspended/expired) require a branded `VerifiedCredentialEvidence` token constructed only by `domain-nurse` verification functions; visit `summary` writes require `MedicalEvidence`-branded payloads. | Type-test proves `nurses.status` cannot be set from outside `domain-nurse` constructors; admin verification flow still passes API E2E; boundary guard extended to credential write sites. | Medium-High |
