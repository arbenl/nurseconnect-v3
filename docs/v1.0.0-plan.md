...
The next numbered program sequence is locked by
...
The current launch decision package is
`docs/runbooks/controlled_launch_execution_readiness.md`. Program source of
truth remains Notion plus the repository specs and runbooks.
...
- [ ] `pnpm launch:readiness` passes on `main`.
- [ ] `pnpm launch:rehearsal` passes against local/test.
- [ ] `LAUNCH_MONITOR_ADMIN_COOKIE='<cookie header>' pnpm launch:monitor -- --url <production-url> --once`
      passes and does not skip admin ops status.
- [ ] `pnpm launch:auth-monitor -- --url <production-url>` passes with the
...
- [ ] Controlled launch execution readiness ledger records GO, HOLD, or NO-GO
      in Notion or a private launch evidence artifact, not in the tracked repo
      runbook.
