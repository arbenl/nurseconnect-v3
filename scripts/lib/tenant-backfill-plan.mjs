...
export function tenantBackfillChecks(organizationId, branchId) {
  ...
  ["assignment_request_org_mismatch", "SELECT count(*) FROM assignments a JOIN service_requests sr ON sr.id=a.request_id WHERE a.organization_id IS NOT NULL AND a.organization_id IS DISTINCT FROM sr.organization_id"],
  ["visit_request_org_mismatch", "SELECT count(*) FROM visits v JOIN assignments a ON a.id=v.assignment_id JOIN service_requests sr ON sr.id=a.request_id WHERE v.organization_id IS NOT NULL AND v.organization_id IS DISTINCT FROM sr.organization_id"],
  ["visit_request_branch_mismatch", "SELECT count(*) FROM visits v JOIN assignments a ON a.id=v.assignment_id JOIN service_requests sr ON sr.id=a.request_id WHERE v.branch_id IS NOT NULL AND v.branch_id IS DISTINCT FROM sr.branch_id"],
  ["event_request_org_mismatch", "SELECT count(*) FROM service_request_events e JOIN service_requests sr ON sr.id=e.request_id WHERE e.organization_id IS NOT NULL AND e.organization_id IS DISTINCT FROM sr.organization_id"],
  ["payment_authorization_request_org_mismatch", "SELECT count(*) FROM payment_authorizations p JOIN service_requests sr ON sr.id=p.request_id WHERE p.organization_id IS NOT NULL AND p.organization_id IS DISTINCT FROM sr.organization_id"],
  ...
  ["nurse_payout_request_org_mismatch", "SELECT count(*) FROM nurse_payouts p JOIN service_requests sr ON sr.id=p.request_id WHERE p.organization_id IS NOT NULL AND p.organization_id IS DISTINCT FROM sr.organization_id"],
  ["nurse_payout_request_nurse_mismatch", "SELECT count(*) FROM nurse_payouts p JOIN service_requests sr ON sr.id=p.request_id WHERE sr.assigned_nurse_user_id IS NULL OR p.nurse_user_id IS DISTINCT FROM sr.assigned_nurse_user_id"],
  ];
}
