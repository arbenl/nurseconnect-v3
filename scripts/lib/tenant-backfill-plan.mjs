...
export function tenantBackfillBatchSql(plan, targetEntityId) {
  return `WITH updated AS (${plan.sql} RETURNING 1), audit AS (
    INSERT INTO admin_audit_logs (actor_user_id, action, target_entity_type, target_entity_id, details)
    SELECT NULL, 'tenant_backfill.batch', 'tenant_backfill', '${targetEntityId}',
      jsonb_build_object('table', '${plan.table}', 'rows', summary.rows, 'source', 'NC-TB-01')
    FROM (SELECT count(*)::int AS rows FROM updated) summary
...
export function tenantBackfillChecks(organizationId, branchId) {
  ...
  ["service_request_non_default_org", `SELECT count(*) FROM service_requests WHERE organization_id IS NOT NULL AND organization_id IS DISTINCT FROM '${organizationId}'`],
  ["service_request_non_default_branch", `SELECT count(*) FROM service_requests WHERE branch_id IS NOT NULL AND branch_id IS DISTINCT FROM '${branchId}'`],
  ["patient_non_default_org", `SELECT count(*) FROM patients WHERE organization_id IS NOT NULL AND organization_id IS DISTINCT FROM '${organizationId}'`],
  ...
  ["request_branch_org_mismatch", "SELECT count(*) FROM service_requests sr JOIN branches b ON b.id=sr.branch_id WHERE sr.organization_id IS NOT NULL AND b.organization_id IS DISTINCT FROM sr.organization_id"],
  ["assignment_request_org_mismatch", "SELECT count(*) FROM assignments a JOIN service_requests sr ON sr.id=a.request_id WHERE a.organization_id IS NOT NULL AND sr.organization_id IS NOT NULL AND a.organization_id IS DISTINCT FROM sr.organization_id"],
  ["visit_request_org_mismatch", "SELECT count(*) FROM visits v JOIN assignments a ON a.id=v.assignment_id JOIN service_requests sr ON sr.id=a.request_id WHERE v.organization_id IS NOT NULL AND sr.organization_id IS NOT NULL AND v.organization_id IS DISTINCT FROM sr.organization_id"],
  ["visit_request_branch_mismatch", "SELECT count(*) FROM visits v JOIN assignments a ON a.id=v.assignment_id JOIN service_requests sr ON sr.id=a.request_id WHERE v.branch_id IS NOT NULL AND sr.branch_id IS NOT NULL AND v.branch_id IS DISTINCT FROM sr.branch_id"],
  ["event_request_org_mismatch", "SELECT count(*) FROM service_request_events e JOIN service_requests sr ON sr.id=e.request_id WHERE e.organization_id IS NOT NULL AND sr.organization_id IS NOT NULL AND e.organization_id IS DISTINCT FROM sr.organization_id"],
  ["payment_authorization_request_org_mismatch", "SELECT count(*) FROM payment_authorizations p JOIN service_requests sr ON sr.id=p.request_id WHERE p.organization_id IS NOT NULL AND sr.organization_id IS NOT NULL AND p.organization_id IS DISTINCT FROM sr.organization_id"],
  ["payment_authorization_patient_mismatch", "SELECT count(*) FROM payment_authorizations p JOIN service_requests sr ON sr.id=p.request_id WHERE p.patient_user_id IS NOT NULL AND p.patient_user_id IS DISTINCT FROM sr.patient_user_id"],
  ["nurse_payout_request_org_mismatch", "SELECT count(*) FROM nurse_payouts p JOIN service_requests sr ON sr.id=p.request_id WHERE p.organization_id IS NOT NULL AND sr.organization_id IS NOT NULL AND p.organization_id IS DISTINCT FROM sr.organization_id"],
  ["nurse_payout_request_nurse_mismatch", "SELECT count(*) FROM nurse_payouts p JOIN service_requests sr ON sr.id=p.request_id WHERE sr.assigned_nurse_user_id IS NOT NULL AND p.nurse_user_id IS DISTINCT FROM sr.assigned_nurse_user_id"],
  ];
}
