...
describe("model-review fallback ladder", () => {
  it("falls through blocked Claude routes to Gemini and skips Copilot", () => {
    const { root, packet, binDir, env } = tempRun();
    try {
      fakeCli(binDir, "claude", "echo quota exhausted >&2\nexit 1");
      fakeCli(binDir, "gemini", "echo GEMINI_OK\nexit 0");
      fakeCli(binDir, "copilot", "echo COPILOT_SHOULD_NOT_RUN\nexit 0");

      execFileSync(process.execPath, [scriptPath, "--packet", packet, "--run-root", root, "--reviewers", "claude48,sonnet46,gemini,copilot", "--fallback-ladder"], { cwd: repoRoot, env });
  ...
  it("exits nonzero when every fallback route is blocked", () => {
    ...
    try {
      ...
      fakeCli(binDir, "copilot", "echo quota exhausted >&2\nexit 1");

      const result = spawnSync(process.execPath, [scriptPath, "--packet", packet, "--run-root", root, "--reviewers", "claude48,gemini,copilot", "--fallback-ladder"], { cwd: repoRoot, encoding: "utf8", env });
      const manifest = JSON.parse(readFileSync(join(root, "reviews/model-review-manifest.json"), "utf8"));
