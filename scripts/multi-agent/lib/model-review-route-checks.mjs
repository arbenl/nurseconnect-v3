...
function accessMarkdown(payload, results, jsonPath) {
  return [
    ...
    "",
    "## Routes",
    ...results.flatMap((result) => ["", `### ${result.reviewer}`, `- status: \`${result.status}\``, `- provider: \`${result.provider}\``, `- model: \`${result.model}\``, result.blockerReason ? `- blocker_reason: \`${inlineText(result.blockerReason)}\`` : null, result.stdout ? `- stdout: \`${inlineText(result.stdout)}\`` : null, result.stderr ? `- stderr: \`${inlineText(result.stderr)}\`` : null].filter(Boolean)),
    "",
    "## Files",
    "",
    `- Raw JSON: \`${jsonPath}\``,
