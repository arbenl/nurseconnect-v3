...
import { manifestSha, writeEvidence } from "./evidence.mjs";
...
export function run(argv = process.argv.slice(2), env = process.env) {
  ...
  try {
    const trackerPath = args.tracker || "docs/plans/current-tracker.md";
    const trackerText = loadTrackerText({ path: trackerPath, base: policyBase, enforcePromotion });
    errors.push(...validateManifest({ manifest, changedFiles, trackerText, enforcePromotion, enforceDiffEvidence }));
  } catch (error) {
  ...
  writeEvidence({ runRoot, manifest, sha, changedFiles, hits, errors });
...
function baseRefAvailable(base) {
  ...
  const result = spawnSync("git", ["rev-parse", "--verify", `${base}^{commit}`], { encoding: "utf8" });
